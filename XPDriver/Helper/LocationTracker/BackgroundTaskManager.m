...
+(instancetype)sharedBackgroundTaskManager{
  //  NSLog(@"sharedBackgroundTaskManager CALLED");
...
-(UIBackgroundTaskIdentifier)beginNewBackgroundTask
{
     ...
     //DLog(@"beginNewBackgroundTask Called");
    if (user_defaults_get_bool(OFFLINE)) {
    ...
    if([application respondsToSelector:@selector(beginBackgroundTaskWithExpirationHandler:)]){
        ...
        else
        {
          ...
          NSLog(@"started background task %lu", (unsigned long)bgTaskId);
            [self.bgTaskIdList addObject:[NSString stringWithFormat:@"%lu",(unsigned long)bgTaskId]];
...
{
     //DLog(@"endAllBackgroundTasks Called");
