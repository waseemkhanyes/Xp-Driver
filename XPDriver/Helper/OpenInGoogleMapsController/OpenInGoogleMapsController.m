...
- (BOOL)isGoogleMapsInstalled {
  ...
  NSURL *callbackURL = [NSURL URLWithString:kGoogleMapsCallbackScheme];
    BOOL isCanOpenSimpleURL    = [_sharedApplication canOpenURL:simpleURL];
     BOOL isCanOpenCallBackURL = [_sharedApplication canOpenURL:callbackURL];
    return isCanOpenSimpleURL || isCanOpenCallBackURL;
...
- (BOOL)fallBackToAppleMapsWithDefinition:(id<GoogleMapsURLSchemable>)definition {
  ...
  NSURL *URLToOpen = [NSURL URLWithString:mapURL];
    if ([_sharedApplication canOpenURL:URLToOpen]) {
           [_sharedApplication openURL:URLToOpen options:@{} completionHandler:^(BOOL success) {
               
           }];
       }
  return [_sharedApplication canOpenURL:URLToOpen];
}
...
- (void)appendMapURLString:(NSMutableString *)mapURL
  ...
  BOOL usingCallback = callbackURL && [_sharedApplication canOpenURL:callbackURL];
    NSString *appCallBackString =  [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"];
  if (usingCallback) {
    [mapURL appendFormat:@"&x-success=%@",
        encodeByAddingPercentEscapes([callbackURL absoluteString])];
    [mapURL appendFormat:@"&x-source=%@",
        encodeByAddingPercentEscapes(appCallBackString)];
  }
}
